<template>
  <div v-if="canEditTeam">
    <!-- Conditionally render the edit team dialog if the user has permission -->
    <Dialog>
      ...
      <DialogContent
        class="dialog-content w-full max-w-[600px] overflow-hidden"
      >
...
onMounted(async () => {
  ...
  if (canEditTeam.value) {
    ...
    if (teamError) {
      ...
      toast({
        title: "There was an error fetching the team details.",
        variant: "destructive",
      });
    } else {
